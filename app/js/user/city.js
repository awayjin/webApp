/**
 * Created by jinwei-258246377@qq.com on 2015/7/4.
 */
...
// 生成省市数组
    function filterProvinceCity(str , push_arr) {
        var As = str.split(",");
...
// console.log(allCodeProvince);

    var allCodeCity = [];
    filterProvinceCity(citysData , allCodeCity);

// 根据省的前两位代码，获取下属城市数组
    function getCity(provinceCode_arg, allCodeCity){
         provinceCode_arg = provinceCode_arg.substr(0,2);
        var selectCity = [];
        for(var i=0; i< allCodeCity.length; i++){
            var provinceCode = allCodeCity[i][0].substr(0,2);
            if(provinceCode_arg == provinceCode){
